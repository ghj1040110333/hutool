...
import com.tools.extra.template.TemplateConfig.ResourceMode;
import com.tools.extra.template.engine.beetl.BeetlEngine;
import com.tools.extra.template.engine.enjoy.EnjoyEngine;
import com.tools.extra.template.engine.freemarker.FreemarkerEngine;
import com.tools.extra.template.engine.rythm.RythmEngine;
import com.tools.extra.template.engine.thymeleaf.ThymeleafEngine;
...
public class TemplateUtilTest {
	...
	public void createEngineTest() {
		...
		Assert.assertEquals("hello,tools", result);
		...
		String result2 = template2.render(Dict.create().set("name", "tools"));
		Assert.assertEquals("hello,tools", result2);
	}
	...
	public void freemarkerEngineTest() {
		...
		result = template.render(Dict.create().set("name", "tools"));
		Assert.assertEquals("hello,tools", result);
	}
	...
	public void thymeleafEngineTest() {
		...
		String result = template.render(Dict.create().set("message", "tools"));
		Assert.assertEquals("<h3>tools</h3>", result);
		...
		result = template.render(Dict.create().set("message", "tools"));
		Assert.assertEquals("<h3>tools</h3>", result);
	}
