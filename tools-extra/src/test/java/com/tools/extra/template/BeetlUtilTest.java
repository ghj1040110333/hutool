...
import com.tools.extra.template.engine.beetl.BeetlUtil;
...
public class BeetlUtilTest {
	...
	public void renderStrTest() throws IOException {
		...
		String result = BeetlUtil.render(template, Dict.create().set("name", "tools"));

		Assert.assertEquals("hello,tools", result);

		String renderFromStr = BeetlUtil.renderFromStr("hello,${name}", Dict.create().set("name", "tools"));
		Assert.assertEquals("hello,tools", renderFromStr);
