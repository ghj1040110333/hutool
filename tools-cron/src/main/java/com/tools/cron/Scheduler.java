...
import com.tools.core.thread.ExecutorBuilder;
...
import com.tools.cron.task.RunnableTask;
	...
	public Scheduler start() {
		...
		try {
			...
			this.threadExecutor = ExecutorBuilder.create().useSynchronousQueue().setThreadFactory(//
					ThreadFactoryBuilder.create().setNamePrefix("tools-cron-").setDaemon(this.daemon).build()//
			).build();
